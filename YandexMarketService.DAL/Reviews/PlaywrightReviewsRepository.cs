...
namespace YandexMarketService.DAL.Reviews
{
    public class PlaywrightReviewsRepository : IReviewsRepository
    {
        ...
        readonly IHubContext<LogsHub> _hubContext;

        public PlaywrightReviewsRepository(IMemoryCache memoryCache, IConfiguration configuration, IHubContext<LogsHub> hubContext)
        {
        ...
        public async Task HandleReviewsAsync(string userId)
        {
            string link_first_rew = String.Empty;
            string? templatesFilePath = _configuration["YANDEX_MARKET_TEMPLATES_FILE_PATH"];

            await _hubContext.Clients.All.SendAsync("ReceiveLogs", "Получен путь файла шаблонов");
            ...
            await _hubContext.Clients.All.SendAsync("ReceiveLogs", "Прочитан файл шаблонов");
            ...
            _ = Task.Run(async () =>
            {
                for (int i = 1; i <= 100; i++)
                {
                    ...
                    await _hubContext.Clients.All.SendAsync("ReceiveLogs", $"Отвечаем на отзыв № {i}");
                }
            });
